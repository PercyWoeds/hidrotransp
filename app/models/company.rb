    class Company < ActiveRecord::Base
      ...
      def get_conductor()
       ...
       def get_pendientes_cliente(fecha1,fecha2,cliente)
          ...
          @facturas = Factura.where([" balance > 0  and  company_id = ? AND fecha >= ? and fecha<= ? and customer_id = ? and tipoventa_id <> '1' ", self.id, "#{fecha1} 00:00:00","#{fecha2} 23:59:59", cliente ]).order(:customer_id,:moneda_id,:fecha)
          return @facturas
      ...
      def get_facturas_day(fecha1,fecha2,moneda)
        ...
        # @boletas = Sellvale.select("fecha,td,cod_prod,ruc,MIN(numero) as minimo, MAX(numero) as maximo,sum(importe2) as total").where(["fecha >= ? and fecha<= ? and td<> ? ", "#{fecha1} 00:00:00","#{fecha2} 23:59:59","N" ]).group(:fecha,:td,:serie,:numero,:cod_prod,:ruc)
         ...
         @boletas = Sellvale.select("fecha,td,serie,numero,ruc,MIN(numero) as minimo, MAX(numero) as maximo,sum(importe2) as total").where(["fecha >= ? and fecha<= ? and td= ?  ", "#{fecha1} 00:00:00","#{fecha2} 23:59:59","F" ]).group(:fecha,:td,:serie,:numero,:ruc)
          ...
          #market
           @boletas = Market.select("fecha,td,serie,ruc,MIN(numero) as minimo, MAX(numero) as maximo,sum(importe) as total").where(["fecha >= ? and fecha<= ? and td= ? ", "#{fecha1} 00:00:00","#{fecha2} 23:59:59","B" ]).group(:fecha,:td,:serie,:ruc)
        # @boletas = Sellvale.select("fecha,td,cod_prod,ruc,MIN(numero) as minimo, MAX(numero) as maximo,sum(importe2) as total").where(["fecha >= ? and fecha<= ? and td<> ? ", "#{fecha1} 00:00:00","#{fecha2} 23:59:59","N" ]).group(:fecha,:td,:serie,:numero,:cod_prod,:ruc)
         ...
         @boletas = Market.select("fecha,td,serie,numero,ruc,MIN(numero) as minimo, MAX(numero) as maximo,sum(importe) as total").where(["fecha >= ? and fecha<= ? and td= ?  ", "#{fecha1} 00:00:00","#{fecha2} 23:59:59","F" ]).group(:fecha,:td,:serie,:numero,:ruc)
        # @boletas = Sellvale.select("fecha,td,cod_prod,ruc,MIN(numero) as minimo, MAX(numero) as maximo,sum(importe2) as total").where(["fecha >= ? and fecha<= ? and td<> ? ", "#{fecha1} 00:00:00","#{fecha2} 23:59:59","N" ]).group(:fecha,:td,:serie,:numero,:cod_prod,:ruc)
      ...
      def get_facturas_by_day_value2(fecha1,fecha2,moneda,value='total')
       ...
       def  get_ventas_combustibles_fecha(fecha1,fecha2) 
        ...
        def  get_ventas_combustibles_fecha_producto0(fecha1,fecha2) 
            ...
            facturas = Sellvale.find_by_sql(['Select sellvales.cod_prod,
                    ...
                    SUM(CAST(importe AS numeric)) AS total 
                      ...
                      GROUP BY sellvales.fecha, sellvales.cod_prod
                      ORDER BY sellvales.fecha, sellvales.cod_prod
        ...
        def  get_ventas_all(fecha1,fecha2) 
          ...
          # order by facturas.fecha, facturas.document_id ', "#{fecha1} 00:00:00","#{fecha2} 23:59:59","2" ])
